...
if(isset($_POST['update_profile'])){
    ...
    if(!empty($name)){
        ...
        $select_users->execute(array($admin_id, $name));
        if($select_users->rowCount() > 0){
        }else{
            $update_users = $conn->prepare("UPDATE `users` SET  name= ? WHERE id= ?");
            $update_users->execute(array($name, $admin_id));
    ...
    if(!empty($email)){
        ...
        $select_users->execute(array($admin_id, $email));
        if($select_users->rowCount() > 0){
        }else{
            $update_users = $conn->prepare("UPDATE `users` SET  email= ? WHERE id= ?");
            $update_users->execute(array($email, $admin_id));
    ...
    if(!empty($image)){
        if($image_size > 2000000){
            ...
            $update_users->execute(array($image, $admin_id));
        ...
        if($old_pass != $empty_pass){
            if($old_pass != $prev_pass){
                ...
                if($new_pass != $empty_pass){
                    ...
                    $update_users->execute(array($cpass, $admin_id));
